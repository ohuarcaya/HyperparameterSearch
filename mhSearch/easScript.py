...
for idModeloPrueba in range(len(estimadorDictionary)):
    ...
    try:
        ...
        gs2 = GeneticSearchCV(estimador, parametros, cv=kf, n_jobs=cpu_count(), verbose=1, scoring='accuracy', refit=False
                     , generations_number=iteraciones, population_size=muestra)
...
for idModeloPrueba in range(len(estimadorDictionary)):
    ...
    try:
        ...
        gs2 = GeneticSearchCV(estimador, parametros, cv=kf, n_jobs=cpu_count(), verbose=1, scoring='accuracy', refit=False
                     , generations_number=iteraciones, population_size=muestra)
        result = gs2.fit(X_train, y_train.BUILDINGID)
        dff = pd.DataFrame(list(gs2.result_cache)).sort_values(['Accuracy'], ascending=False).reset_index(drop=True)
